...
def fluency_index(request):
    ...
    for unit in units:
        unit["count"] = len(
            Flashcard.objects.values("unit_id").filter(unit_id=unit["id"])
        )
    ...
    return render(request, template_name, {"units": units, "pages": pages})
...
def submit_score_report(request):
    ...
    if module == "fluency_untimed":
        results = FluencyUntimedScore(
            score=score, unit_id=unit, deck_id=deck, date=timezone.now()
        )
        results.save()
...
def fluency_timed_view(request, quiz_id, deck_id):
    ...
    if request.method == "GET":
        ...
        flashcard_ids = Flashcard.objects.filter(
            unit_id=quiz_id, deck_id=deck_id
        ).values_list("id", flat=True)

        flashcard_set = list(
            Flashcard.objects.filter(unit_id=quiz_id, deck_id=deck_id).all()
        )

        for flashcard in flashcard_set:
            choices = list(
                Choice.objects.filter(flashcard_id=flashcard.id).values_list(
                    "id", "choice_text", "is_correct"
                )
            )
            flashcard.flashcard_choices = choices
            task = list(
                Task.objects.filter(pk=flashcard.task_list_item_id).values_list(
                    "id", "task_name", "task_list_description", "certification"
                )
            )
...
def fluency_flashcard_inspector(request, flashcard_id):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            "fluency_flashcard_inspector.html",
            {
                "flashcard": flashcard,
