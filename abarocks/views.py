...
def redirect_root(request):
    ...
    pinned_posts = Post.objects.filter(
        published_date__lte=timezone.now(), is_pinned=True
...
def redirect_study(request):
    pages = Pages.objects.order_by("order")
    modules = Module.objects.filter(published_date__isnull=False).order_by(
        "module_order"
    )
    return render(
        request,
...
def redirect_error_report(request):
    ...
    message = (
        "Hey there - a user reported a problem in the <b><u>"
        + module.upper()
        + "</b></u> module, for <b>item ID "
        + question_id
        + "</b>, which reads: <br /><br /> "
        + item
        + " <br /><br />"
        + description
        + "<br/><br/> They categorized the bug as follows "
        + categories[0]
    )

    subject = (
        "ABA.Rocks - Problem Reported in " + module.upper() + " module, #" + item_id
    )

    send_mail(
        subject,
        "Hey there - a user reported a problem on question ID "
        + question_id
        + " with this description: "
        + description
        + "and the following categories were "
        + categories[0],
